...
// UPDATED: Notifies the backend of a session termination before removing client configurations
...
async function uploadProfileImage() {
    ...
    // Convert file object to a base64 Data URL data stream
    reader.onload = async function (e) {
        ...
        try {
            ...
            if (response.ok) {
                alert("Your profile image was successfully saved to the backend!");
            } else {
                alert("Failed to save image to backend.");
            }
        } catch (error) {
            console.error("Error uploading profile image:", error);
            alert("Could not connect to server to upload image.");
        }
    };
...
async function exportPDF(id) {
    ...
    // Dynamic Server Image Generation Step
    if (r.owner) {
        try {
            // Request the specific base64 string directly from the backend API
            const response = await fetch(`${BASE_URL}/users/${r.owner}/image`);
            const data = await response.json();
            
            if (data && data.imageBase64) {
                // Insert base64 stream directly into document layout coordinates
                doc.addImage(data.imageBase64, "PNG", 140, 30, 45, 45);
            }
        } catch (error) {
            console.error("Failed to load owner image from backend:", error);
        }
    }
