...
const Severity = Object.freeze({
    ...
    // Mapování obrázků pro jednotlivé ownery (Base64 formát nebo URL)
    // Nahraďte tyhle ukázkové base64 řetězce svými skutečnými obrázky
    // Mapování na reálné soubory ve tvé složce imag
    const OWNER_IMAGES = {
        "Eliáš": "imag/elias.png",
        "Švarc": "imag/svarc.png",
        "Gras": "imag/gras.png"
    };
    
    // Pomocná funkce, která načte lokální PNG a převede ho na formát pro jsPDF
    ...
    // Naplnění selectů uživateli dynamicky
    populateOwnerSelects();
    renderReports();
    }
    
    // Pomocná funkce pro naplnění select boxů uživateli z pole USERS
    function populateOwnerSelects() {
        const selects = [reportOwner, editOwner, filterOwner];
        selects.forEach(select => {
            if (!select) return;
            // Promaže staré optiony kromě první (výchozí)
    ...
    reports.push(new Report(
        ...
        reportOwner.value // Uložení ownera
    ));
    ...
    const owner = filterOwner.value.toLowerCase().trim(); // Nový filtr pro ownera
    ...
    editOwner.value = r.owner || ""; // Načtení ownera do editace
    ...
    reports[i].owner = editOwner.value; // Uložení změny ownera
    ...
    async function exportPDF(id){
        ...
        // KONTROLA A VLOŽENÍ REÁLNÉHO OBRÁZKU
        if (r.owner && OWNER_IMAGES[r.owner]) {
            try {
                // Počkáme, až se lokální PNG obrázek načte z disku do paměti
                const imgElement = await loadImage(OWNER_IMAGES[r.owner]);
                
                // Vložíme obrázek do PDF (pozice x=150, y=30, šířka=40, výška=40)
                doc.addImage(imgElement, 'PNG', 150, 30, 40, 40);
            } catch (error) {
                console.error("Nepodařilo se načíst obrázek ownera ze složky imag:", error);
            }
        }
